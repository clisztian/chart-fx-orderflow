...
public class TradeBubbleRendererSample extends Application {

    Random rng = new Random(23);
    List<Color> my_colors;
    private DefaultDataSet buySet;
    private DefaultDataSet sellSet;
...
//        final DefaultDataSet sellSet = new DefaultDataSet("SELLS");
        ...
        sellSet = new DefaultDataSet("SELLS");
        ...
        for(int i = 0; i < 400000; i ++) {
            ...
            n_millis = rng.nextInt(5000);
            now = now.plus(n_millis, ChronoUnit.MILLIS);
            t = (double)now.toInstant(ZoneOffset.UTC).toEpochMilli();

            label = "SELL" + i;
            markerSize = rng.nextDouble()*2;
            size = "markerSize=" + 10 * Math.sqrt(markerSize) + "; index=" + i + ";";

            Color rand_col_2 = Color.MAGENTA;
            red = 186;
            green = 155;
            blue = 255;
        ...
        double[] timestamp = new double[sellSet.getDataCount()];
        double[] price_heat = new double[1000];
        double[][] z = new double[1000][sellSet.getDataCount()];
        ...
        for(int i = 0; i < sellSet.getDataCount(); i++) {
            timestamp[i] = sellSet.get(DataSet.DIM_X, i);
        ...
        reductionAlgorithm.setMinPointPixelDistance(200);
        ...
        chart.getRenderers().setAll(contourRenderer, spreadRenderer, tradeRenderer);
        tradeRenderer.getDatasets().addAll(buySet, sellSet);
...
//        VBox.setVgrow(pane, Priority.ALWAYS);
    ...
    public void addTradeSpread() {
        ...
        if(rng.nextFloat() < .1f) {
            ...
            int i = sellSet.getDataCount();

            String label = "SELL" + i;
            double markerSize = rng.nextDouble()*2;
            String size = "markerSize=" + 10 * Math.sqrt(markerSize) + "; index=" + i + ";";

            Color rand_col_2 = Color.MAGENTA;
            int red = (int)(rand_col_2.getRed()*255);
            int green = (int)(rand_col_2.getGreen()*255);
            int blue = (int)(rand_col_2.getBlue()*255);
