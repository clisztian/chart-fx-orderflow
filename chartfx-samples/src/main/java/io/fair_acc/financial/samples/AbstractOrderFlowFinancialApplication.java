...
import io.fair_acc.financial.samples.service.TickOhlcvReplayDataSet;
 ...
 * @author afischer
 */
public abstract class AbstractOrderFlowFinancialApplication extends Application {
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractOrderFlowFinancialApplication.class);
    ...
    protected Chart getDefaultFinancialTestChart(final String theme) {
        ...
        if (resource.startsWith("REALTIME")) {
            try {
                ...
                ohlcvDataSet = new TickOhlcvReplayDataSet(
                        ...
                        replayFromCal,
                        consolidationAddons,
                        "/home/lisztian/FXProjects/chart-fx/chartfx-samples/src/main/resources/data/CH0012005267_20220209.csv");

                System.out.println("Name: " + ohlcvDataSet.getName());
    ...
    protected void showPredefinedTimeRange(String dateIntervalPattern, OhlcvDataSet ohlcvDataSet,
        ...
        try {
            ...
            System.out.println("Predefined time range: " + dateIntervalPattern + " (" + fromIdx + " - " + toIdx + ")");
    ...
    private void pauseResumeTimer() {
        if (!timerActivated) {
            startTimer();
        } else if (ohlcvDataSet instanceof TickOhlcvReplayDataSet) {
            ((TickOhlcvReplayDataSet) ohlcvDataSet).pauseResume();
        }
    }
    ...
    private void startTimer() {
        if (ohlcvDataSet instanceof TickOhlcvReplayDataSet) {
            TickOhlcvReplayDataSet realtimeDataSet = (TickOhlcvReplayDataSet) ohlcvDataSet;
    ...
    private void stopTimer() {
        if (timerActivated && ohlcvDataSet instanceof TickOhlcvReplayDataSet) {
